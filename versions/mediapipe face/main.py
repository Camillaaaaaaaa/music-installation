...
class Main:
    ...
    def start_camera(self):
        ...
        with self.FaceDetector.create_from_options(options) as detector:
            while cap.isOpened():
                ...
                if self.visualizations.score_width == 0:
                    self.visualizations.set_width(int(image_width - self.score_offset_x - 50))
                ...
                if self.results :
                    print(self.results)
                    for index,person in enumerate(self.results.detections):
                        instrument = (self.current_instrument + index) % len(self.instruments)
                        head_pos = self.video_analysis.update_head_pos(image_height, image_width,person)
                        head_width = self.video_analysis.get_face_size(image_height, image_width,person)
                ...
                annotated_image = visualize(image_copy, results)
    ...
    def print_result(self, result: mp.tasks.vision.FaceDetectorResult, output_image: mp.Image, timestamp_ms: int):
        self.results=result
